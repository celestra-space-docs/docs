...
4.  **Escape Velocity**: If the orbit's semi-major axis $ a $ is infinite (hypothetical parabolic escape orbit), the Vis-Viva equation yields the escape velocity formula.
    ...
    - $v_{esc}$: This represents the escape velocity — the minimum speed required for an object to escape the gravitational field of a celestial body.
...
- **Kinetic Energy**: The kinetic energy ($KE$) required to reach escape velocity is given by $\frac{1}{2}mv_{esc}^2$, where $m$ is the mass of the object.
